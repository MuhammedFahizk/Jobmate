    ...
    <div className="bg-background min-h-screen font-body overflow-hidden py-12 md:py-20">
      <div className="max-w-[1000px] mx-auto px-6">
        ...
        <div className="text-center flex flex-col items-center gap-6 mb-16">
          ...
          <h1 className="font-display text-4xl font-bold tracking-tight text-foreground max-w-xl leading-tight">
            Build Your Career with <span className="bg-gradient-to-r from-primary-700 to-primary-500 bg-clip-text text-transparent">M Cube Services</span>
          </h1>
        ...
        >
          {openings.map((job) => (
            <motion.div
              ...
              className="bg-white p-6 sm:p-8 rounded-card border border-border shadow-card flex flex-col gap-6 relative overflow-hidden"
            >
              {/* Type Badge top-right */}
              <div className="absolute top-6 right-6 inline-flex px-3 py-1 rounded-pill bg-primary-100 text-primary-700 text-xs font-semibold border border-primary-100">
                {job.type}
              </div>
 
              {/* Title & Metadata */}
              <div className="flex flex-col gap-1.5 pr-28">
                <span className="font-body text-[10px] uppercase tracking-wider font-semibold text-primary-700">
                  {job.department}
                </span>
                <h3 className="font-display font-bold text-foreground text-lg sm:text-xl leading-tight">
                  {job.title}
                </h3>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 font-body text-xs text-muted mt-1.5">
                  <span className="flex items-center gap-1">
                    <MapPin size={14} className="text-primary-500" /> {job.location}
                  </span>
                  <span className="flex items-center gap-1">
                    <DollarSign size={14} className="text-primary-500" /> {job.salary}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="font-body text-xs font-bold text-foreground">Key Requirements:</span>
                  <ul className="list-disc pl-5 font-body text-xs text-muted flex flex-col gap-1">
